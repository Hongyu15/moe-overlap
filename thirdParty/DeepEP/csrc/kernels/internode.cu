...
// 在普通深度学习模型中，数据大小是固定的（如224*224的图片），但是在MoE模型中，Token是动态路由。
// 问题来了，如果此时GPU0要发送500个Token给GPU5，但是GPU5根本不知道该往哪里接，也不知道准备多大内存
// 因此，必须先有一个notify通知阶段，GPU0：GPU5，我一会要给你发送500个Token，你准备好Buffer[500].
template <bool kLowLatencyMode, int kNumRDMARanks>   //  kNumRDMARanks: 一共多少台机器
__global__ void notify_dispatch(const int* num_tokens_per_rank,     // 发货清单（数组）：当前这张gpu准备发往其他每个gpu的token数量，num_tokens_per_rank[5]=100，说明有100个token发给5号gpu.
                                int* moe_recv_counter_mapped,    // 收货计数器，通常指向那个一块“映射内存”，这意味着CPU和远程GPU都能看到。当notify_dispatch算清楚全集群一共发送多少Token后，将总数写进这个地址。
                                int num_ranks,     // 整个集群的显卡数
                                const int* num_tokens_per_rdma_rank,   // 发往各台device的token数量
                                int* moe_recv_rdma_counter_mapped,   // 当这台机器收集到所有其他机器发来的token后，会把总数写在这里
                                const int* num_tokens_per_expert,    // 记录了发送给全集群中每一个专家的token数
                                ...
                                int num_channels,
                                int expert_alignment,          //  每个专家接收到的token数必须是某个整数值(比如8、16)的整数倍
                                const int rdma_clean_offset,   // rdma通信的元数据(Metadata)在缓冲区中起始下标，即offset
                                const int rdma_num_int_clean,  // rdma通信的元数据需要清零的size大小，即几个int
                                const int nvl_clean_offset,    // nvl机内通信的元数据在缓冲区的起始下标offset
                                const int nvl_num_int_clean,   // nvl机内通信需要清零的int数量
    ...
    // 在开始搬运token之前，指挥官必须确保：上一轮通信的数据全部处理完，且当前网络管道是空的。
    if (sm_id == 0) {      // sm_id == 0, 指挥官 Block
        // Communication with others
        // Global barrier: the first warp does intra-node sync, the second warp does internode sync
        EP_DEVICE_ASSERT(num_warps > 1);   // 必须至少2个warp，一个盯着跨机同步，一个盯着机内
        EP_DEVICE_ASSERT(kNumRDMARanks <= num_threads);   // 线程数不少于机器数，保证一个线程盯着一台机器
