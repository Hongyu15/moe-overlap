...
//  Buffer结构体是整个DeepEP的核心，是通信库的“中央控制塔”
struct Buffer {
...
private:
    ...
    // NVLink Buffer    机内通信的地址簿
    int64_t num_nvl_bytes;
    void* buffer_ptrs[NUM_MAX_NVL_PEERS] = {nullptr};    //  核心，CPU数组，数组里8个指针，分别对应当前机箱8张卡在UVA映射后的虚拟地址
    void** buffer_ptrs_gpu = nullptr;   //  buffer_ptrs 数组在GPU显存中的备份。因为Kernel不能访问CPU数组，所以初始化时，CPU会把8个地址拷贝到显存里，然后GPU上我们把这个 buffer_ptrs_gpu 传给 kernel.

    // NVSHMEM Buffer
    int64_t num_rdma_bytes;
    void* rdma_buffer_ptr = nullptr;   //  为IB/RoCE网络（RDMA）准备的缓冲区，当token需要跨机通信的时候，先到这排队
