...
class SimpleMoE(nn.Module):   # 简化版 MoE 模型，包含路由、专家计算、通信后端和overlap配置
    def __init__(
        ...
        # Reserve SMs for DeepEP communication, and use the rest for Triton compute.
        self.triton_num_sms = max(1, 108 - deepep_num_sms)
        ...
        if self.comm_backend == "deepep":
            ...
            if self.expert_kernel == "triton":
                raise ValueError("Mode torch_a2a + triton is disabled for now.")
        # Reused across forward passes so Nsight traces keep stable stream IDs (single allocation per device).
        self._deepep_compute_stream: torch.cuda.Stream | None = None
        self._deepep_comm_stream: torch.cuda.Stream | None = None
        self._deepep_streams_device: torch.device | None = None

    def _ensure_deepep_streams(self, device: torch.device) -> tuple[torch.cuda.Stream | None, torch.cuda.Stream | None]:
        if device.type != "cuda":
            return None, None
        if (
            self._deepep_compute_stream is None
            or self._deepep_streams_device != device
        ):
            self._deepep_compute_stream = torch.cuda.Stream(device=device)
            self._deepep_comm_stream = torch.cuda.Stream(device=device)
            self._deepep_streams_device = device
        return self._deepep_compute_stream, self._deepep_comm_stream
    ...
    def _run_routed_experts_torch(self, tokens: torch.Tensor, expert_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for expert_id in range(self.num_local_routed_experts):
            ...
            if n == 0:
                continue
            counts[expert_id] += n
            # Under autocast, experts may return bf16/fp16 while `zeros_like(tokens)` follows token dtype (often fp32 after LayerNorm).
            out[mask] = self.routed_experts[expert_id](tokens[mask]).to(out.dtype)
        return out, counts
    ...
    def _run_routed_experts_grouped(self, tokens: torch.Tensor, expert_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        b1 = b1.to(gemm_dtype)
        b2 = b2.to(gemm_dtype)
        hidden = persistent_grouped_gemm(grouped_tokens_gemm, b1, offsets, num_sms=self.triton_num_sms)
        hidden = F.gelu(hidden)
        grouped_out = persistent_grouped_gemm(hidden, b2, offsets, num_sms=self.triton_num_sms)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, RouterStats]:
        ...
        if self.comm_backend == "torch_a2a":
            ...
            compute_stream, comm_stream = self._ensure_deepep_streams(x.device)
            ...
            def dispatch_chunk(chunk_idx: int, previous_event: object | None = None):
                ...
                comm_ctx = torch.cuda.stream(comm_stream) if comm_stream is not None else contextlib.nullcontext()
                with comm_ctx:
            ...
            for chunk_idx in range(num_chunks):
                ...
                with stream_ctx:
                    ...
                    compute_done = torch.cuda.Event() if compute_stream is not None else None
                    if compute_done is not None:
                        compute_done.record(compute_stream)

                comm_ctx = torch.cuda.stream(comm_stream) if comm_stream is not None else contextlib.nullcontext()
                with comm_ctx:
                    if comm_stream is not None and compute_done is not None:
            ...
            if comm_stream is not None:
                torch.cuda.current_stream(device=x.device).wait_stream(comm_stream)
...
#  生成tokens输入   [local_num_tokens, batch_size * seq_len]
def synthetic_batch(
